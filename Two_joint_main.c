...
MotorParams revolute_motor_params = {
    .R = 5.237f,      // Ohms (from your measurements)
    .L = 0.176f,      // Henry
    .Kt = 1.551f,     // Nm/A
    .Ke = 1.574f,     // V/(rad/s)
    .J = 0.009f,      // kg*m²
...
DisturbanceComp revolute_disturbance = {
    .mp = 0.5f,       // Plotter mass in kg
...
DisturbanceComp prismatic_disturbance = {
    .mp = 0.5f,       // Same masses
    .mc = 0.5f,
    .g = 9.81f,
    .T = 0.05f,
...
FeedforwardComp revolute_feedforward = {
    .T = 0.05f,       // 50ms filter time constant
    .enabled = true   // Enable by default
};

FeedforwardComp prismatic_feedforward = {
    .T = 0.05f,
    .enabled = true
};
...
SystemParams system_params = {
    .lambda = 0.2f    // Example value, adjust as needed
};
...
float RevoluteFeedforward(float theta_ref, float omega_ref, float alpha_ref, MotorParams* motor, SystemParams* system);
...
void UpdateRevoluteTuningParameters(float pos_kp, float pos_ki, float pos_kd, float vel_kp, float vel_ki, float vel_kd);
void UpdatePrismaticTuningParameters(float pos_kp, float pos_ki, float pos_kd, float vel_kp, float vel_ki, float vel_kd);
...
float CascadeController_Update(CascadeController* controller,
    ...
    controller->velocity_setpoint = VelocityPID_Update(
        &controller->position_pid,
        position_setpoint,
        position_feedback
    );
...
//                0.001f);  // 1ms sample time
    ...
    MotorModelKalman_Init(&revolute_motor_filter, 0.001f,
                        ...
                        revolute_motor_params.R, revolute_motor_params.L,
...
//                0.001f);  // 1ms sample time
    ...
    MotorModelKalman_Init(&prismatic_motor_filter, 0.001f,
                        ...
                        prismatic_motor_params.R, prismatic_motor_params.L,
                        Q_increased, R_decreased);
...
float RevoluteFeedforward(float theta_ref, float omega_ref, float alpha_ref,
    ...
    float v_feedforward = motor->R * current_required + motor->Ke * omega_ref * motor->n;
...
void TestVelocityBidirectional(void) {
    ...
    static float velocity_setpoint[] = {
        		        1.0f,                // Starting position (0 degrees)
        		        1.0f,       // 30 degrees
        		//        0.0f,                // Back to 0
        		//        M_PI/4.0f,           // 45 degrees
        		//        0.0f,                // Back to 0
        ...
        static const uint8_t num_setpoints = sizeof(velocity_setpoint) / sizeof(velocity_setpoint[0]);
...
void MoveRevoluteToPosition(float target_position_rad) {
    Trapezoid_SetTarget(&revolute_trap, revolute_encoder.position, target_position_rad);
    move_in_progress = 1;
}
...
float RevoluteDisturbanceComp(float theta, DisturbanceComp* comp, MotorParams* params) {
    if (!comp->enabled) return 0.0f;

    // Vin = (nr*R/kt) * (mp-mc)*g*r*cos(θ)
    float mp_minus_mc = comp->mp - comp->mc;
    float cos_theta = cosf(theta);
    float disturbance_torque = mp_minus_mc * comp->g * system_params.lambda * cos_theta;
...
float PrismaticDisturbanceComp(float theta, DisturbanceComp* comp, MotorParams* params) {
    if (!comp->enabled) return 0.0f;
...
float PrismaticFeedforward(float pos_setpoint, float vel_setpoint, float accel_setpoint,
                          MotorParams* params, SystemParams* sys_params) {
    if (!prismatic_feedforward.enabled) return 0.0f;

    float J_term = (params->J + (params->n * params->n) / (params->r * params->r)) * accel_setpoint;
    float B_term = params->B * vel_setpoint;
...
void RevoluteControlLoop(void) {
    ...
    float position_setpoint = move_in_progress ? revolute_trap.position : revolute_tuning.position_setpoint;
    float velocity_setpoint = move_in_progress ? revolute_trap.velocity : revolute_cascade.velocity_setpoint;
    float acceleration_setpoint = move_in_progress ? revolute_trap.acceleration : 0.0f;

    float control_signal = revolute_cascade.current_setpoint;
    MotorModelKalman_Estimate(&revolute_motor_filter, control_signal, revolute_encoder.position);

    float filtered_velocity = revolute_motor_filter.velocity;

    if (!revolute_tuning.control_enabled) {
        SetRevoluteMotorPWM(0.0f, revolute_tuning.max_voltage, revolute_tuning.pwm_period);
        return;
    }

    float compensated_velocity = ApplyPhaseLead(filtered_velocity, 0.5f, 0.6f);

    float v_feedback = CascadeController_Update(
        &revolute_cascade,
        position_setpoint,
    ...
    float v_feedforward = RevoluteFeedforward(
        position_setpoint,
        velocity_setpoint,
        acceleration_setpoint,
        &revolute_motor_params,
        &system_params
    );

    float v_disturbance = RevoluteDisturbanceComp(
        revolute_encoder.position,
        &revolute_disturbance,
        &revolute_motor_params
    );

    float control_signal_output = v_feedback + v_feedforward + v_disturbance;

    SetRevoluteMotorPWM(control_signal_output, revolute_tuning.max_voltage, revolute_tuning.pwm_period);
...
void PrismaticControlLoop(void) {
    ...
    float revolute_position = latest_revolute_position + 0.0495;
    float revolute_velocity = revolute_encoder.speed;

    float target_displacement = revolute_position * 1.0f;
    float revolute_change = revolute_position - prev_revolute_position;
    prev_revolute_position = revolute_position;

    prismatic_tuning.position_setpoint = -target_displacement;

    if (fabsf(revolute_change) > 0.000f) {
        float displacement_rate = -fabsf(revolute_velocity) * 1.0f;
//        if(0 < displacement_rate < 40){
//        	displacement_rate = 4;
//        }
//        else if(displacement_rate == 0){
//        	displacement_rate = 0;
//        }
//        else{
//        	displacement_rate = -35;
//        }
        if (revolute_change > 0) {
            prismatic_cascade.velocity_setpoint = displacement_rate;
        } else {
            prismatic_cascade.velocity_setpoint = -displacement_rate;
        }
    }
...
//    }
    ...
    float v_disturbance = PrismaticDisturbanceComp(
        revolute_position,
    ...
    float v_feedback = CascadeController_Update(&prismatic_cascade,
        ...
        prismatic_encoder.speed + revolute_encoder.speed,
		50.0f);

    float control_signal_output = v_feedback + v_feedforward + v_disturbance;

    SetPrismaticMotorPWM(control_signal_output, prismatic_tuning.max_voltage, prismatic_tuning.pwm_period);
}
