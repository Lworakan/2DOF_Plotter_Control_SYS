...
void GenerateStepTestData(float *position, float *actual_velocity,
		float *input_voltage, int size, float amplitude, float step_time);
void GenerateRampTestData(float *position, float *actual_velocity,
		float *input_voltage, int size, float slope, float max_amplitude);
void GenerateSineTestData(float *position, float *actual_velocity,
		float *input_voltage, int size, float amplitude, float frequency);

/* Testing function to compare filters */
void CompareFilters(float *position_measurements, float *input_voltage,
		float *actual_velocity, int size);
/* USER CODE END PFP */
...
void Cascade_Init(CascadeController *cascade, float pos_kp, float pos_ki,
		float pos_kd, float vel_kp, float vel_ki, float vel_kd, float dt) {
	PID_Init(&cascade->position_controller, pos_kp, pos_ki, pos_kd, -50.0f,
			50.0f, dt);

	PID_Init(&cascade->velocity_controller, vel_kp, vel_ki, vel_kd, -24.0f,
			24.0f, dt);


	VelocityPID_Init(&cascade->velocity_pid, vel_kp, vel_ki, vel_kd, dt);

	cascade->velocity_setpoint = 0.0f;
	cascade->current_setpoint = 0.0f;
	cascade->max_velocity = 30.0f; // pris
...
float VelocityPID_Update(VelocityPID* pid, float setpoint, float measurement) {
    ...
    float derivative_term = (pid->Kp * pid->Kd / pid->T) * (error - 2.0f * pid->prev_error + pid->prev_prev_error);
...
float Cascade_Update(CascadeController *cascade, float pos_ref,
    ...
    // Velocity setpoint limiting (unchanged)
    float max_velocity_change = 0.2f;

    if (raw_velocity_setpoint - cascade->velocity_setpoint
            > max_velocity_change) {
        cascade->velocity_setpoint += max_velocity_change;
    } else if (cascade->velocity_setpoint - raw_velocity_setpoint
            > max_velocity_change) {
        cascade->velocity_setpoint -= max_velocity_change;
    } else {
    ...
    // Use VelocityPID_Update instead of PID_Update
    cascade->current_setpoint = VelocityPID_Update(&cascade->velocity_pid,
            cascade->velocity_setpoint, vel_actual);

    if (cascade->current_setpoint > 24.0f) {
    	cascade->current_setpoint = cascade->max_velocity;
        }
        if (cascade->current_setpoint < -24.0f ){
        	cascade->current_setpoint = -24.0f;
        }
...
void SetMotorPWM(float voltage, float max_voltage, uint32_t period) {
	...
	__HAL_TIM_SET_COMPARE(&htim1, TIM_CHANNEL_1, pwm_value); // 1 pris
...
//void SetMotorPWM(float voltage, float max_voltage, uint32_t period)  {
//
//    const uint32_t MIN_PWM_THRESHOLD = 800;
//
//    const float DEADBAND_PERCENT = 0.05f;  // 5% deadband
//
//    if (voltage > max_voltage) {
//        voltage = max_voltage;
...
//    }
//
//    GPIO_PinState dir_state = (voltage >= 0) ? GPIO_PIN_RESET : GPIO_PIN_SET;
//
//    float abs_voltage = fabsf(voltage);
//
//    if (abs_voltage > 0.0f) {
//        abs_voltage = DEADBAND_PERCENT * max_voltage +
//                      (1.0f - DEADBAND_PERCENT) * abs_voltage;
//    }
//
//    uint32_t pwm_value = (uint32_t)(abs_voltage / max_voltage * period);
//
//    if (pwm_value > 0 && pwm_value < MIN_PWM_THRESHOLD) {
//        if (abs_voltage < 0.05f * max_voltage) {
//
//            pwm_value = 0;
//        } else {
//            pwm_value = MIN_PWM_THRESHOLD;
//        }
//    }
//
...
//
//
//    HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, dir_state);
//}
void InitializeTuning(void) {
	...
	tuning.max_velocity = 30.0f;
	tuning.max_voltage = 24.0f;
	tuning.pwm_period = 1000;
...
// pris
	motorParams.R = 2.252824525f;
	motorParams.L = 0.07603942161f;
	motorParams.Ke = 0.05618266023f;
	motorParams.Jm = 0.0000925625527f;
	motorParams.Bm = 0.000000001001456027f;
	motorParams.Kt = 0.055f;
	...
	Cascade_Init(&cascade, tuning.position_kp, tuning.position_ki,
			tuning.position_kd, tuning.velocity_kp, tuning.velocity_ki,
			tuning.velocity_kd, 0.001f);  // 10ms sample time
...
//    HAL_Delay(1000);
	...
	float Q_increased = 0.0001f;
	float R_decreased = 1.0f;
...
float GenerateTestReference(void) {
	...
	return tuning.test_amplitude
			* sinf(2.0f * M_PI * tuning.test_frequency * time_sec);
}
...
float revolute_feedforward(float theta_ref, float omega_ref, float alpha_ref,
	...
	float T_motor_inertia = motor->Jm * alpha_ref * motor->n_gear
			* motor->n_gear;
	...
	float v_feedforward = motor->R * current_required
			+ motor->Ke * omega_ref * motor->n_gear;
...
void robot_start_trajectoryy(void) {
	...
	if (!initialized) {
		...
		target_params.max_velocity = 30.0f;           // Target velocity (rad/s)
...
//    Kalman();
	...
	motor_model_velocity = motorFilter.velocity * 0.25f;
...
//    float control_signal_output;
	float compensated_velocity = ApplyPhaseLead(motor_model_velocity, 0.3f,
			0.7f);

	// Use full cascade control
	float position_reference = tuning.position_setpoint;
//        cascade.velocity_setpoint = 1.0;
	float velocity_reference = cascade.velocity_setpoint;
	float acceleration_reference = 4.0f;

	float v_feedforward = revolute_feedforward(position_reference,
			velocity_reference, acceleration_reference, &motorParams,
			&systemParams);

	float v_feedback = Cascade_Update(&cascade, position_reference,
			encoder.position, encoder.speed);

//        control_signal_output = v_feedback + v_feedforward;

	control_signal_output = v_feedback;
...
void UpdateTuningParameters(float pos_kp, float pos_ki, float pos_kd,
	...
	VelocityPID_Init(&cascade.velocity_pid, vel_kp, vel_ki, vel_kd, 0.001f);
...
//	const float velocity_setpoints[] = { -30.0f, 30.0f, -30.0f, 30.0f, -30.0f,
//			30.0f };


	const float velocity_setpoints[] = { 1.0f, 1.0f, 0.0f, -0.0f, -1.0f,
			-1.0f };
	const uint8_t num_setpoints = sizeof(velocity_setpoints)
			/ sizeof(velocity_setpoints[0]);

	// Change velocity every 3 seconds (3000 ms)
	if (HAL_GetTick() - last_velocity_change > 1500) {
...
int main(void)
{
  ...
  /* USER CODE BEGIN Init */
	Encoder_Init(&encoder, &htim3); // 2rev
  /* USER CODE END Init */
...
//
//		static uint32_t current_Hz = 0;
//		static uint32_t current_H_2 = 0;
//		static int start = 0;
//		if (HAL_GetTick() - current_Hz > 500) {
//			current_Hz = HAL_GetTick();
//
//			start += 1;
//
