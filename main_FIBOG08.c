...
FastDrawingMode drawing_mode = {
    .fast_mode_active = 0,
    .position_tolerance = 0.0008f,  // 0.8mm tolerance
...
void MoveToCartesianPositionSTM32(float x, float y) {
	...
	float revolute_time = CalculateTrajectoryTime(revolute_distance, 1.0f,
			0.4f);
	float prismatic_time = CalculateTrajectoryTime(prismatic_distance, 30.0f,
			5.0f);
	float sync_time = fmaxf(revolute_time, prismatic_time);
	...
	if (prismatic_velocity > 30.0f)
		prismatic_velocity = 30.0f;
...
CartesianPosition GetSmoothedPosition(LookAheadBuffer* buffer, float smoothing_factor) {
    if (buffer->count < 2) {
...
void CalculateVelocityFeedforward(CartesianPosition current, CartesianPosition target,
                                 float dt, float speed_factor,
...
void CartesianToJointVelocity(float cart_vel_x, float cart_vel_y,
    ...
    if (fabs(r) > 0.005f) { // Avoid division by very small numbers
        ...
        if (fabs(det) > 0.001f) {
            *joint_vel_revolute = (cart_vel_x * (-sin_theta) + cart_vel_y * cos_theta) / r;
            *joint_vel_prismatic = -COUPLING_RATIO * (cart_vel_x * cos_theta + cart_vel_y * sin_theta);
        } else {
        ...
        *joint_vel_revolute = 0;
        *joint_vel_prismatic = -COUPLING_RATIO * sqrt(cart_vel_x * cart_vel_x + cart_vel_y * cart_vel_y);
    }
    ...
    if (fabs(*joint_vel_revolute) > max_revolute_vel) {
        *joint_vel_revolute = (*joint_vel_revolute > 0) ? max_revolute_vel : -max_revolute_vel;
    }
    if (fabs(*joint_vel_prismatic) > max_prismatic_vel) {
        *joint_vel_prismatic = (*joint_vel_prismatic > 0) ? max_prismatic_vel : -max_prismatic_vel;
    }
}
...
void AdjustPIDForDrawing(uint8_t fast_mode) {
    if (fast_mode) {
        ...
        UpdateRevoluteTuningParameters(5.0f, 0.000000000000000000000000001f, 0.0f, 0.8475f, 0.00625f, 0.0f);
...
//		//underscore (offset)
				...
				{ 0.005000f, 0.0000000f },
				{ 0.010000f, 0.0000000f },
				{ 0.010000f, 0.0050000f },
				{ 0.010000f, 0.0100000f },
				{ 0.010000f, 0.0150000f },
				{ 0.010000f, 0.0200000f },
				{ 0.005000f, 0.0200000f },
				{ 0.000000f, 0.0200000f },
				{ 0.000000f, 0.0150000f },
				{ 0.000000f, 0.0100000f },
				{ 0.000000f, 0.0050000f },
				{ 0.000000f, 0.0000000f },
...
void WriteFIBO_G08_Optimized(void) {
    ...
    if (point_index < total_points) {
        ...
        CalculateVelocityFeedforward(current_pos, target_position,
                                   update_interval / 1000.0f,
                                   target_point.speed_factor,
...
//	total_points = total_points*8;
	...
	if (IsCoordinatedMoveComplete()
			&& (abs(joints.revolute_angle-revolute_encoder.position)  < 0.25f) && (abs(joints.prismatic_pos-prismatic_encoder.position)  < 0.25f)) {
		if (point_index < total_points) {
...
//	UpdatePrismaticTuningParameters(8.0f, 0.02f, 0.5f, 0.2f, 0.003f, 0.0f);
	...
	UpdateRevoluteTuningParameters(5.0f, 0.0000000000000000000000000001f,
				0.000000f, 0.8475f, 0.00625f, 0.0f);
	//	UpdatePrismaticTuningParameters(25.575f, 0.0755f, 2.8f, 1.2f, 0.6f, 0.0f);
//		UpdatePrismaticTuningParameters(25.575f, 0.0755f, 2.5f, 0.63f, 0.3f, 0.0f);
	UpdatePrismaticTuningParameters(25.575f, 0.0755f, 2.5f, 0.63f, 0.02f, 0.0f);
...
void TestCoordinatedMovementFixed(void) {
	...
	static CoordPosition coord_positions[] = { { 0.0f, 0.0f },   // Both at zero
			{ M_PI / 4.0f,  -0.05 * 2.0 * M_PI / 0.08 }, // Both move together
			{ M_PI / 2.0f,  -0.10 * 2.0 * M_PI / 0.08 },  // Both move together
			{ M_PI / 6.0f,  -0.15 * 2.0 * M_PI / 0.08 }                 // Both return to zero
	};
...
void MoveToCoordinatedPositionSynced(float revolute_target,
	...
	SetTrajectoryParameters(&revolute_trajectory, revolute_velocity, 0.4f);
	SetTrajectoryParameters(&prismatic_trajectory, prismatic_velocity, 1.0f);
