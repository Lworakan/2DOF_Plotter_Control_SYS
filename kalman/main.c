...
void Encoder_Init(EncoderData* enc, TIM_HandleTypeDef* htim) {
    ...
    float Q = 0.001f;         // Process noise (adjust based on testing)
    float R = 4.0f;           // Measurement noise (adjust based on testing)
...
void ControlLoop(void) {
    ...
    float filtered_position = MotorKalman_GetPosition(&motorKalman);
    float filtered_velocity = MotorKalman_GetVelocity(&motorKalman);

    float estimated_load_torque = MotorKalman_GetLoadTorque(&motorKalman);
    ...
    float control_signal = v_feedback + v_feedforward;
...
int main(void)
{
  ...
  UpdateTuningParameters(0.0f, 0.0f, 0.0f, 1.2f,10.0f, 0.0f);
