...
uint32_t proximity_sensor_state = 0;
uint32_t home_step_timer = 0;
uint32_t home_step_duration = 100;
float home_position_revolute = 0.0f;
float home_position_prismatic = 0.0f;
uint8_t home_found = 0;
uint8_t HOME = 0;
uint8_t Emergency = 0;
...
HomeState home_state = HOME_IDLE;
...
void ProcessHomeSequence(void);
void InitiateHomeRequest(void);
void ResetEncodersToZero(void);
void ResetAllControlParameters(void);
/* USER CODE END PFP */
...
float CascadeController_Update(CascadeController *controller,
	...
	// OUTER LOOP: Position control at 500Hz (every 2ms) using PID_Update
	if ((current_time - controller->last_position_update_time)
			>= controller->position_loop_period_ms) {
		...
		// Update velocity setpoint for inner loop
	...
	// INNER LOOP: Velocity control at 1000Hz (every 1ms) using VelocityPID_Update
	if ((current_time - controller->last_velocity_update_time)
			>= controller->velocity_loop_period_ms) {

		// Update velocity controller (inner loop) using VelocityPID_Update
...
void ProcessHomeSequence(void) {
	...
	case HOME_IDLE:
		if (HOME == 1) {
			HOME = 0; // Clear the flag immediately
			InitiateHomeRequest();
		}
		break;

	case HOME_REQUESTED:
		EmergencyStopTrajectory(&revolute_trajectory);
		EmergencyStopTrajectory(&prismatic_trajectory);
		DisableControl();
		...
		home_state = HOME_STOPPING;
		home_step_timer = current_time;
		break;

	case HOME_STOPPING:
		if (current_time - home_step_timer >= home_step_duration) {
			home_state = HOME_RESETTING;
			home_step_timer = current_time;
		}
		break;

	case HOME_RESETTING:
		// Step 3: Reset all parameters (non-blocking)
		ResetEncodersToZero();
		ResetAllControlParameters();
	...
	case HOME_COMPLETE:
		// Step 4: Re-enable control and finish
		if (current_time - home_step_timer >= home_step_duration) {
			EnableControl();
			home_found = 1;
...
void InitiateHomeRequest(void) {
	if (home_state == HOME_IDLE) {
		home_state = HOME_REQUESTED;
		home_step_timer = HAL_GetTick();
	}
}

void ResetEncodersToZero(void) {
	home_position_revolute = revolute_encoder.position;
	home_position_prismatic = prismatic_encoder.position;

	revolute_encoder.count = 0;
	revolute_encoder.last_count = 0;
	revolute_encoder.last_count_for_speed = 0;
	revolute_encoder.position = 0.0f;
	revolute_encoder.speed = 0.0f;
	revolute_encoder.speed_filtered = 0.0f;

	prismatic_encoder.count = 0;
	prismatic_encoder.last_count = 0;
	prismatic_encoder.last_count_for_speed = 0;
	prismatic_encoder.position = 0.0f;
	prismatic_encoder.speed = 0.0f;
	prismatic_encoder.speed_filtered = 0.0f;
...
void ResetAllControlParameters(void) {
	...
	revolute_cascade.position_setpoint = 0.0f;
	revolute_cascade.velocity_setpoint = 0.0f;
	revolute_cascade.current_setpoint = 0.0f;

	prismatic_cascade.position_setpoint = 0.0f;
	...
	revolute_cascade.position_pid.last_time = HAL_GetTick();
	...
	prismatic_cascade.position_pid.last_time = HAL_GetTick();
	...
	MotorModelKalman_Reset(&revolute_motor_filter);
	MotorModelKalman_Reset(&prismatic_motor_filter);

	revolute_trajectory.active = 0;
	revolute_trajectory.phase = PHASE_IDLE;
	revolute_trajectory.ang_pos = 0.0f;
	revolute_trajectory.ang_velo = 0.0f;
	revolute_trajectory.ang_acc = 0.0f;
	revolute_trajectory.t[0] = 0ULL;
	revolute_trajectory.t[1] = 0ULL;
	revolute_trajectory.t_elapsed = 0.0f;

	prismatic_trajectory.active = 0;
	prismatic_trajectory.phase = PHASE_IDLE;
	prismatic_trajectory.ang_pos = 0.0f;
	prismatic_trajectory.ang_velo = 0.0f;
	prismatic_trajectory.ang_acc = 0.0f;
	prismatic_trajectory.t[0] = 0ULL;
	prismatic_trajectory.t[1] = 0ULL;
	prismatic_trajectory.t_elapsed = 0.0f;

	// Reset compensation parameters
	prismatic_raw_setpoint = 0.0f;
	prismatic_compensation_offset = 0.0f;
	prev_revolute_position = 0.0f;
...
void CheckProximityAndHome(void) {
	proximity_sensor_state = HAL_GPIO_ReadPin(GPIOB, GPIO_PIN_2);

	if (proximity_sensor_state == 1 && home_state == HOME_IDLE) {
		HOME = 1; // Request home sequence
	}
}
...
uint8_t GetSystemHomeStatus(void) {
	if (Emergency == 1)
		return 0; // Emergency state
	if (home_state != HOME_IDLE)
		return 1; // Homing in progress
	if (!home_found)
		return 2; // Not homed yet
...
const char* GetHomeStatusString(void) {
	switch (home_state) {
	case HOME_IDLE:
		return home_found ? "READY" : "NOT_HOMED";
	case HOME_REQUESTED:
		return "HOME_REQ";
	case HOME_STOPPING:
		return "STOPPING";
...
uint8_t SafeMoveIfReady(float revolute_target, float prismatic_target) {
	if (GetSystemHomeStatus() == 4 && home_state == HOME_IDLE) {
		MoveToCoordinatedPosition(revolute_target, prismatic_target);
		return 1; // Movement started
	}
	return 0;
}

void TriggerManualHome(void) {
	if (home_state == HOME_IDLE) {
		HOME = 1;
	}
}
...
void PrismaticControlLoop(void) {
	...
	float v_feedback = CascadeController_Update(&prismatic_cascade,
			prismatic_tuning.position_setpoint, prismatic_encoder.position,
			prismatic_encoder.speed, max_velocity);
...
void InitializeTrajectoryGenerators(void) {
	...
	prismatic_trajectory.desire_av = 30.0f;   // 0.5 rad/s max velocity
	prismatic_trajectory.desire_ac = 15.0f;   // 2.0 rad/s² acceleration
	prismatic_trajectory.active = 0;
	prismatic_trajectory.phase = PHASE_IDLE;
}
...
void SetTrajectoryParameters(Trajectory *trj, float max_velocity,
		float acceleration) {
	if (!trj->active) {  // Only allow changes when trajectory is not running
...
void RevoluteControlLoopWithTrajectory(void) {
	...
	// Determine setpoints based on whether a trajectory is active
	float position_setpoint;
	float velocity_setpoint;
	float acceleration_setpoint;
	...
	if (check_status(&revolute_trajectory)) {
		...
		// Optional: Add trajectory debugging info
		// You can log: revolute_trajectory.phase, revolute_trajectory.profile_type

	} else {
		// Using direct setpoints (manual control)
		position_setpoint = revolute_tuning.position_setpoint;
		velocity_setpoint = revolute_cascade.velocity_setpoint;
		acceleration_setpoint = 0.0f;
	}
...
void PrismaticControlLoopWithTrajectory(void) {
	...
	if (check_status(&prismatic_trajectory)) {
		...
		// Using existing setpoints (manual control or oscillation)
...
// Test function with automatic profile selection demonstration
...
//		}
		...
		if (home_state != HOME_STOPPING && home_state != HOME_RESETTING) {
			RevoluteControlLoopWithTrajectory();
			PrismaticControlLoopWithTrajectory();

		}

		if (home_found && Emergency == 0 && home_state == HOME_IDLE) {
			...
			if (!movement_done && (HAL_GetTick() - startup_time > 10000)) {
//				SetTrajectoryParameters(&revolute_trajectory, 1.0f, 0.4f);
//				MoveRevoluteToPositionNew(M_PI / 4.0f);
				SetTrajectoryParameters(&prismatic_trajectory, 40.0f, 15.0f);
				MovePrismaticToPositionNew(0.3 * 6 / 8 / RAD_PER_DEGREE);
