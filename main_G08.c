...
static CouplingParams coupling_params = {
    .coupling_coeff = 1.0f,      // Direct coupling (revolute affects prismatic 1:1)
...
float CascadeController_Update(CascadeController *controller,
		...
		float velocity_feedback, float max_velocity_limit, float tra_velocity_setpoint) {
	...
	if ((current_time - controller->last_position_update_time)
		...
		controller->velocity_setpoint = new_velocity_setpoint+ tra_velocity_setpoint;
...
//	}
	...
	float v_feedback = CascadeController_Update(&revolute_cascade,
			position_setpoint, revolute_encoder.position,
			filtered_velocity, 1.0f, velocity_setpoint);
...
CartesianPosition forward_kinematics(JointPosition joints) {
    ...
    float actual_prismatic_extension = joints.prismatic_pos - joints.revolute_angle;
...
void MoveToCartesianPositionSynced(CartesianPosition target) {
    ...
    float prismatic_velocity = prismatic_distance / (sync_time * 0.8f);
     ...
     SetTrajectoryParameters(&revolute_trajectory, revolute_velocity, TRAJECTORY_MAX_ACCEL_REVOLUTE);
     SetTrajectoryParameters(&prismatic_trajectory, prismatic_velocity, TRAJECTORY_MAX_ACCEL_PRISMATIC);
     MoveRevoluteToPositionNew(joints.revolute_angle);
     MovePrismaticToPositionNew(joints.prismatic_pos);
}
...
void MoveToCartesianPositionSTM32(float x, float y) {
    ...
    float revolute_distance = fabs(joints.revolute_angle - revolute_encoder.position);
    float prismatic_distance = fabs(joints.prismatic_pos - prismatic_encoder.position);

    float revolute_time = CalculateTrajectoryTime(revolute_distance, 1.0f, 0.4f);
    float prismatic_time = CalculateTrajectoryTime(prismatic_distance, 30.0f, 15.0f);
    float sync_time = fmaxf(revolute_time, prismatic_time);
    ...
    if (revolute_velocity > 1.0f) revolute_velocity = 1.0f;
    if (prismatic_velocity > 30.0f) prismatic_velocity = 30.0f;

    // Use your existing trajectory functions
    SetTrajectoryParameters(&revolute_trajectory, revolute_velocity, 0.4f);
    SetTrajectoryParameters(&prismatic_trajectory, prismatic_velocity, 15.0f);
 ...
 */
void DrawCartesianLineSTM32(CartesianPosition start, CartesianPosition end, int num_points) {
...
void ExecuteCoordPositionsWithInverseKinematics(void) {
    ...
    if (IsCoordinatedMoveComplete() && (HAL_GetTick() - last_move_time > 3000)) {
        ...
        JointPosition joints = {current_coord.revolute_pos, current_coord.prismatic_pos};
...
void SimpleCartesianInverseTest(void) {
    ...
    static CartesianPosition simple_pattern[] = {
        {0.0f, 0.05f},      // Forward 5cm
        {0.05f, 0.05f},     // Right 5cm, forward 5cm
        {0.05f, 0.0f},      // Right 5cm
        {0.0f, 0.0f}        // Back to origin
    };

    static const uint8_t num_positions = sizeof(simple_pattern) / sizeof(simple_pattern[0]);
...
CartesianPosition ConvertGcodeToRobot(float gcode_x, float gcode_y) {
    ...
    float distance = sqrt(robot_pos.x * robot_pos.x + robot_pos.y * robot_pos.y);
...
void WriteFIBO_G08_FromGcode(void) {
    ...
    if (IsCoordinatedMoveComplete() && (HAL_GetTick() - last_move_time > 2000)) {
        if (command_index < FIBO_COMMANDS_COUNT) {
            ...
            if (current_command.pen_state == PEN_DOWN && !pen_servo_commanded) {
                ...
                return;
            } else if (current_command.pen_state == PEN_UP && pen_servo_commanded) {
            ...
            if (ValidateCartesianTarget(current_command.x, current_command.y)) {
                ...
                MoveToCartesianPositionSTM32(current_command.x, current_command.y);
...
static CartesianPosition FIBO_G08_complete[] = {
    // Letter F
    {0.00f, 0.01f}, {0.00f, 0.03f}, {0.015f, 0.03f}, {0.00f, 0.02f}, {0.01f, 0.02f},
    // Move to I
    {0.02f, 0.01f},
    // Letter I
    {0.02f, 0.01f}, {0.03f, 0.01f}, {0.025f, 0.01f}, {0.025f, 0.03f}, {0.02f, 0.03f}, {0.03f, 0.03f},
    // Move to B
    {0.04f, 0.01f},
    // Letter B
    {0.04f, 0.01f}, {0.04f, 0.03f}, {0.055f, 0.03f}, {0.055f, 0.025f}, {0.045f, 0.02f},
    {0.055f, 0.02f}, {0.055f, 0.015f}, {0.04f, 0.01f},
    // Move to O
    {0.065f, 0.01f},
    // Letter O
    {0.065f, 0.015f}, {0.065f, 0.025f}, {0.075f, 0.03f}, {0.085f, 0.025f},
    {0.085f, 0.015f}, {0.075f, 0.01f}, {0.065f, 0.015f},
    // Underscore
    {0.095f, 0.005f}, {0.125f, 0.005f},
    // Move to G
    {0.135f, 0.01f},
    // Letter G
    {0.15f, 0.02f}, {0.145f, 0.03f}, {0.135f, 0.025f}, {0.135f, 0.015f},
    {0.145f, 0.01f}, {0.15f, 0.015f}, {0.145f, 0.02f},
    // Move to 0
    {0.16f, 0.01f},
    // Number 0
    {0.16f, 0.015f}, {0.16f, 0.025f}, {0.17f, 0.03f}, {0.18f, 0.025f},
    {0.18f, 0.015f}, {0.17f, 0.01f}, {0.16f, 0.015f},
    // Move to 8
    {0.19f, 0.01f},
    // Number 8
    {0.195f, 0.02f}, {0.19f, 0.025f}, {0.195f, 0.03f}, {0.20f, 0.025f},
    {0.195f, 0.02f}, {0.19f, 0.015f}, {0.195f, 0.01f}, {0.20f, 0.015f}, {0.195f, 0.02f},
    // End
    {0.21f, 0.02f}
};

// Function to execute FIBO_G08 writing pattern
    ...
    if (IsCoordinatedMoveComplete() && (HAL_GetTick() - last_move_time > 2000)) {
        if (point_index < total_points) {
            ...
            if (ValidateCartesianTarget(target.x, target.y)) {
                ...
                // printf("Writing point %d/%d: (%.3f, %.3f)\n",
                //        point_index + 1, total_points, target.x, target.y);
            }
...
//        };
    ...
    if (IsCoordinatedMoveComplete() && (HAL_GetTick() - last_move_time > 3000)) {
        ...
        MoveToCoordinatedPositionSynced(coord_positions[move_index].revolute_pos,
...
void TestCoordinatedMovement(void) {
	static uint32_t last_move_time = 0;
 //
	static float prismatic_positions[] = { -0.025 * 2.0 * M_PI / 0.08, -0.10 * 2.0 * M_PI / 0.08, -0.2 * 2.0 * M_PI / 0.08 }; // ต้องสร้างอันนี้ขึ้นมาอีกอัน
//	static uint8_t move_index = 0;
	static int8_t direction = 1;  // 1 = forward, -1 = backward
	static const uint8_t num_positions = 3;

	if (IsCoordinatedMoveComplete() && (HAL_GetTick() - last_move_time > 2500)) {
...
//	}
void MoveToCoordinatedPositionSynced(float revolute_target, float prismatic_target) {
    float revolute_distance = fabsf(revolute_target - revolute_encoder.position);
    float prismatic_distance = fabsf(prismatic_target - prismatic_encoder.position);

    float revolute_time = CalculateTrajectoryTime(revolute_distance, 1.0f, 0.4f);
    float prismatic_time = CalculateTrajectoryTime(prismatic_distance, 40.0f, 20.0f);
