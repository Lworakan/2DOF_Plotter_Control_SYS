...
// Control tolerances from presentation requirements
...
CartesianPosition forward_kinematics(JointPosition joints) {
    ...
    float effective_radius = linear_prismatic - (joints.revolute_angle / COUPLING_RATIO);
...
JointPosition inverse_kinematics(CartesianPosition target) {
    ...
    float required_linear_displacement = r + (theta / COUPLING_RATIO);

    // Convert linear displacement to joint angle (matching your system)
    joints.prismatic_pos = required_linear_displacement * COUPLING_RATIO;


    joints.prismatic_pos += theta; // Add revolute angle for compensation

    // Iterative refinement for better accuracy (optional)
...
void TestCoordinatedMovement(void) {
	static uint32_t last_move_time = 0;
 //
	static float prismatic_positions[] = { -0.05 * 2.0 * M_PI / 0.08, -0.10 * 2.0 * M_PI / 0.08, -0.2 * 2.0 * M_PI / 0.08 }; // ต้องสร้างอันนี้ขึ้นมาอีกอัน
//	static uint8_t move_index = 0;
