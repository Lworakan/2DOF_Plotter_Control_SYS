...
void SmoothCircleDrawer_Update(SmoothCircleDrawer *drawer) {
	...
	if (drawer->circle_phase == 0)
		angular_acceleration = drawer->max_acceleration / drawer->radius;
	else if (drawer->circle_phase == 2)
		angular_acceleration = -drawer->max_acceleration / drawer->radius;
...
//    prismatic_cascade.velocity_setpoint = drawer->prismatic_trajectory_velocity;
	...
	drawer->revolute_position_error = drawer->revolute_trajectory_position
			- revolute_encoder.position;
	drawer->revolute_velocity_error = drawer->revolute_trajectory_velocity
			- revolute_encoder.speed;
...
void SmoothCircleDrawer_GetTrajectoryValues(SmoothCircleDrawer *drawer,
		float *rev_pos, float *rev_vel, float *rev_acc, float *pris_pos,
		float *pris_vel, float *pris_acc) {
...
void SmoothCircleDrawer_GetErrors(SmoothCircleDrawer *drawer,
		float *rev_pos_err, float *rev_vel_err, float *pris_pos_err,
		float *pris_vel_err) {
...
void StartPrismaticOscillation(void) {
	...
	prismatic_tuning.position_setpoint =
			prismatic_target_positions[current_target_index];
...
void GenerateSimpleSinusoidalTrajectory(void) {
	...
	revolute_tuning.position_setpoint = amplitude
			* sinf(angular_freq * elapsed_s);
	prismatic_tuning.position_setpoint = amplitude
...
void SmoothCircularMotion(void) {
	...
	float target_revolute_vel = -circle_radius * angular_velocity
			* sinf(current_angle);
	float target_prismatic_vel = circle_radius * angular_velocity
			* cosf(current_angle);

	float target_revolute_acc = -circle_radius * angular_velocity
			* angular_velocity * cosf(current_angle);
	float target_prismatic_acc = -circle_radius * angular_velocity
			* angular_velocity * sinf(current_angle);
