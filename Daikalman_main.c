...
// For DC motor model filter
float process_noise = 0.01f;  // Lower than your current setting
float meas_noise = 0.5f;       // Higher to reduce noise sensitivity
...
void Encoder_Init(EncoderData* enc, TIM_HandleTypeDef* htim) {
    ...
    float dt = 2.0f;         // 10ms sampling time
    float J = 0.009f;         // Motor inertia (kg*m²)
    float b = 0.199f;         // Viscous friction coefficient (N*m*s/rad)
    float Kt = 1.551f;        // Torque constant (N*m/A)
    float Ke = 1.574f;        // Back-EMF constant (V*s/rad)
    float Ra = 5.237f;        // Armature resistance (Ohms)
    float La = 0.176f;        // Armature inductance (H)
//    float Q = 0.9f;         // Process noise (adjust based on testing)
//    float R = 0.1f;           // Measurement noise (adjust based on testing)

//    MotorKalman_Init(&motorKalman, dt, J, b, Kt, Ke, Ra, La, Q, R);
}
...
void InitializeTuning(void) {
    ...
    motorParams.R = 5.237f;        // Ohms
    motorParams.L = 0.176f;        // Henry
    motorParams.Ke = 1.574f;       // V·s/rad
    motorParams.Kt = 1.551f;       // N·m/A
    motorParams.Jm = 0.009f;       // kg·m²
    motorParams.Bm = 0.199f;       // N·m·s/rad
    ...
    systemParams.J_arm = 0.05f;    // kg·m² (estimate)
    systemParams.c_revolute = 0.05f; // N·m·s/rad (estimate)
...
//    HAL_Delay(1000);
    ...
    float dt = 0.01f;         // 10ms sampling time
        float J = 0.009f;         // Motor inertia (kg*m²)
        float b = 0.199f;         // Viscous friction coefficient (N*m*s/rad)
        float Kt = 1.551f;        // Torque constant (N*m/A)
        float Ke = 1.574f;        // Back-EMF constant (V*s/rad)
        float Ra = 5.237f;        // Armature resistance (Ohms)
        float La = 0.176f;        // Armature inductance (H)
        float Q = 0.01f;         // Process noise (adjust based on testing)
        float R = 0.6;           // Measurement noise (adjust based on testing)
        ...
        MotorModelKalman_Init(&motorFilter, 0.002f,  // 10ms sampling time
                                 ...
                                 0.01f, 0.6f);  // Process and measurement noise

           // Initialize kinematic Kalman filter
...
void ControlLoop(void) {
    ...
    Encoder_CalculateSpeed(&encoder);
        ...
        float kinematic_model_velocity = kinematicFilter.velocity;
        ...
        float filtered_velocity = use_motor_model ?
                                 motorFilter.velocity :
                                 kinematicFilter.velocity;
...
//	test_velocity = 1.0f;
  ...
  if (HAL_GetTick() - last_switch > 3000) {
	  dir_switch = 1;
//	HAL_GPIO_WritePin(GPIOB, GPIO_PIN_1, dir_switch );
...
void MotorModelKalman_Init(MotorModelKalman* filter, float dt, float J, float b,
    ...
    /* Fill state matrix based on DC motor physics */
...
int main(void)
{
  ...
  UpdateTuningParameters(0.0f, 0.0f, 0.0f, 0.2f,5.0f, 0.0f);
  ...
  while (1){
    ...
    /* USER CODE BEGIN 3 */
	  static uint32_t current_Hz = 0;
	     if (HAL_GetTick() - current_Hz > 2) {
	         current_Hz = HAL_GetTick();

	         // Toggle between filters every 10 seconds
	         if (HAL_GetTick() - filter_switch_time > 5) {
	         ...
	         ControlLoop(); static uint32_t current_Hz = 0;
	         if (HAL_GetTick() - current_Hz > 2) {
	             current_Hz = HAL_GetTick();
